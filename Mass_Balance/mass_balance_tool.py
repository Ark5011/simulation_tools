...
# TRANSACTION_HISTORY_URL = 'http://localhost:5003/archive'
...
@app.route('/')
def index():
    return render_template('mass_balance_fe.html', input_data={})

def format_value(value, fmt):
    return fmt.format(value) if isinstance(value, (float, int)) else value

@app.route('/mass_balance_tool', methods=['POST'])
def mass_balance_tool():
    try:

        # Set default values for all expected inputs
        default_values = {
            'primaryairAH': 0.0,
            'primaryair': 0.0,
            'primaryinletT': 0.0,
        ...
        # Retrieve form data and handle empty fields
        data = {}
...
def generate_excel():
    try:
        ...
        data_from_mbtool = session.get('calculated_data', {})
        ...
        excel_data = [
            ...
            ['SFB Inlet RH', round(data_from_mbtool.get('sfb_inlet_rh', ''), 2), '%'],
            ['Outlet Air AH', round(data_from_mbtool.get('outlet_air_ah', ''), 2), 'g/kg'],
            ['Outlet Air RH', round(data_from_mbtool.get('outlet_air_rh', ''), 2), '%'],
            ['Dummy Outlet Air RH', round(data_from_mbtool.get('dummy_outlet_air_rh', ''), 2), 'g/kg'],
            [' '],
            ['Temperatures'],
            ...
            ['Dummy Outlet Air T', round(data_from_mbtool.get('dummy_outlet_air_t', ''), 2), '°C'],
            [' '],
            ['Airflows'],
            ...
            ['Barometric Pressure', round(data_from_mbtool.get('barometric_pressure', ''), 2), 'kPa'],
            [' '],
            ['Concentrate'],
            ...
            ['Production Rate', round(data_from_mbtool.get('production_rate', ''), 2), 'kg/h'],
            [' '],
        ...
        response.headers["Content-Disposition"] = "attachment; filename=mass_balance.xlsx"
